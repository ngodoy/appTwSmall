...
//ToString()
    ...
    static function formatosSQl() {
        return "yyyy-mm-dd hh:mm:ss";
    }

    static function formatosNormal() {
        return "dd-mm-yyyy";
    }
...
//_reset()
    ...
    function Years($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_years == 0)
            $this->Span("years", $dateStart, $dateStop);
        return $this->_years;
    }

    function Months($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_months == 0)
            $this->Span("months", $dateStart, $dateStop);
        return $this->_months;
    }

    function WeekDays($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_weekdays == 0)
            $this->Span("weekdays", $dateStart, $dateStop);
        return $this->_weekdays;
    }

    function Days($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_days == 0)
            $this->Span("days", $dateStart, $dateStop);
        return $this->_days;
    }

    function Weeks($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_weeks == 0)
            $this->Span("weeks", $dateStart, $dateStop);
        return $this->_weeks;
    }

    function Quarters($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_quarters == 0)
            $this->Span("quarters", $dateStart, $dateStop);
        return $this->_quarters;
    }

    function Hours($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_hours == 0)
            $this->Span("hours", $dateStart, $dateStop);
        return $this->_seconds();
    }

    function Minutes($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_minutes == 0)
            $this->Span("minutes", $dateStart, $dateStop);
        return $this->_minutes();
    }

    function Seconds($dateStart = "", $dateStop = "") {
        if ($dateStart || $dateStop || $this->_seconds == 0)
            $this->Span("seconds", $dateStart, $dateStop);
        return $this->_seconds;
    }
